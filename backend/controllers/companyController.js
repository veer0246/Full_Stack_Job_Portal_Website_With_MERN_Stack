...
// get company details according to login user filling company form ===================================================
...
export const getCompanyById = async (req, res) => {
    try {
        const companyId = req.params.id; // params to capture dynamic value from the url
        const company = await Company.findById(companyId);
        if (!company) {
            return res.status(404).json({ message: "Company not found.", success: false })
        }
        return res.status(200).json({ company, success: false })
    } catch (error) {
        console.log(error)
    }
}
